...
Channel.builder = function(name) {
  var protocols = [],
      builder = {
        ...
        build: function() {
          ...
          stack.on('message',  onMessage(channel));
          stack.on('viewChanged', onViewChanged(channel));
          stack.on('memberAdded', onMemberAdded(channel));
          stack.on('memberRemoved', onMemberRemoved(channel));
          return channel;
        }
      };
  ...
  function onMessage(channel) {
    return function(message) {
      channel.emit('message', message.body, message);
    };
  }
   ...
   * and metadata about what has changed.
   */
  function onMemberAdded(channel) {
    return function(member, view) {
      channel.emit('memberAdded', member, view);
    };
  }
   ...
   * and metadata about what has changed.
   */
  function onMemberRemoved(channel) {
    return function(member, view) {
      channel.emit('memberRemoved', member, view);
    };
  }
...
Channel.prototype.connect = function(callback) {
  ...
  if (this.state === Channel.State.CONNECTED) {
    return Q(this).nodeify(callback);
  } else {
...
Channel.prototype.send = function(recipient, message, callback) {
  ...
  if (this.state !== Channel.State.CONNECTED) {
    return Q.reject('Channel not connected').nodeify(callback);
  }
